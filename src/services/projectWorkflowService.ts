...
export const projectWorkflowService = {
  ...
  async updateProjectPhase(projectId: string, updateData: {
    currentPhase: string;
    currentSubstep: string;
    ...
    try {
      const response = await api.put(`/project-workflow/${projectId}/phase`, updateData);
      return response.data.project;
    } catch (error) {
  ...
  async toggleProjectSubstep(projectId: string, phase: string, substepName: string): Promise<ProjectWorkflow> {
    try {
